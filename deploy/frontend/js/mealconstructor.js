...
function saveAsMeal() {
  ...
  const method = editingId ? 'PUT' : 'POST';
  const url = editingId ? `${API_BASE_URL}/meals/${editingId}` : `${API_BASE_URL}/meals/${selectedDate}/add`;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (editingId && editingType && token) {
    ...
    if (editingType === 'recipes') {
      url = `${API_BASE_URL}/recipes/${editingId}`;
    } else if (editingType === 'meals') {
      url = `${API_BASE_URL}/meals/${editingId}`;
