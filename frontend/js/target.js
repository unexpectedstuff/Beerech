...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Destructure and convert necessary user data to numbers for calculation
  ...
  // Get references to HTML elements where the results will be displayed
  ...
  // Check if all required data is available, if not, display empty values
  if (!(gender && age && weight && height && activity && goal)) {
    caloriesEl.textContent = "—"; // Display a placeholder if data is missing
    return;
  }
  ...
  // Calculate macronutrient breakdown based on final calories
